...
def gen_cxr_seg_dataset_ls(data_dir, 
    ...
    mask_dir_ls = [os.path.join(data_dir, 'masks', mask_name) for mask_name in mask_name_ls] 
    img_path_ls = load_file_path(img_dir, IMG_EXTENSION)
    mask_dir_dict = {mask_name:os.path.join(data_dir, 'masks', mask_name) for mask_name in mask_name_ls}
    mask_name_path_dict = {mask_name:load_file_path(mask_dir, IMG_EXTENSION) for mask_name, mask_dir in mask_dir_dict.items()}
    ...
    if csv_path is None:
        ...
        if split is None:
            dataset_fname_ls = split_df[fname_col].values
        else:
            dataset_fname_ls = split_df[split_df.split==split][fname_col].values
    
    img_mask_ls = match_data_and_mask(img_path_ls, mask_name_path_dict, dataset_fname_ls)
